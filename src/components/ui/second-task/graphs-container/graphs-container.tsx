...
const GraphsContainer: React.FC = () => {
    ...
    const { countriesData, isLoading } = useCountriesData(selectedCountry, selectedStartYear, selectedEndYear);
    const highestEmissionYear = countriesData?.reduce((prev, current) => (prev.Total > current.Total) ? prev : current);
                ...
                <LoadingIndicator /> : (
                    <div className="container flex flex-col w-2/3 bg-slate-200 my-8 py-8 rounded h-fit p-8">
                        ...
                        <div className="py-8 self-center flex flex-row">
                            ...
                            <PieChart countryData={countriesData} />
                        </div>
                        <div className="py-8 self-center">
                            <LineChart countryData={countriesData} />
                        </div>
